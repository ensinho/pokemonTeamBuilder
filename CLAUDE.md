...
- **Hand-written `:hover` is gated by the build** — `scripts/lib/postcssHoverGate.mjs` (in `postcss.config.js`, after Tailwind) wraps every `:hover` in `(hover: hover) and (pointer: fine)` and copies its fills/colours to `:active` for touch. Write hover normally; don't gate it by hand. Tailwind theme colours are `themeColor()` functions so opacity modifiers (`bg-primary/15`) actually compile.
- **Dialogs:** `.modal-scrim` > `.modal-panel` with `ref={useModalA11y(onClose)}` (a callback ref). The hook owns Escape, focus, drag-to-dismiss on phones (below 640px every panel is a sheet) and the exit animation. Don't hand-roll any of those.
- **Design system v2 primitives** (`src/styles/interactions.css`, 2026-09-24): `<Switch>` for any yes/no setting, `<Loader>` for any loading state, `<RollingNumber>` for a figure the user is changing, `useShinyBurst` for a reward the user's own click earned, `confirmAction()` (never `window.confirm`) before something irreversible, and an undo toast for anything reversible. `.segmented` / `.tabs` indicators travel via CSS anchor positioning — keep items as direct children of the track. **A theme picked by the user goes through `chooseTheme()` (`src/store/themeChoice.js`)** — it applies, animates and saves to the account; calling `changeTheme` from a control skips the save and the next sign-in reverts it.
...
- **`/verify-ui` skill** (`.claude/skills/verify-ui/`) — the evidence a UI claim needs before it is made: gates (tests, build, lint delta against the recorded baseline, drift audits) and the headless-Edge browser pass at 1440/390 × dark/light, with mid-animation frames and CDP touch gestures.
