...
const RandomGeneratorView = ({ colors, generations, db, userId }) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (seen) {
                    try { localStorage.setItem(introStorageKey, '1'); } catch (_) {}
                }
    ...
    const handleToggleGuideLayout = useCallback(() => {
        setIsGuideLayoutExpanded((prev) => {
            const next = !prev;
            try { localStorage.setItem(guideLayoutStorageKey, next ? '1' : '0'); } catch (_) {}
            return next;
        });
    }, [guideLayoutStorageKey]);
    ...
    const handleToggleFiltersLayout = useCallback(() => {
        setIsFiltersExpanded((prev) => {
            const next = !prev;
            try { localStorage.setItem(filtersLayoutStorageKey, next ? '1' : '0'); } catch (_) {}
            return next;
        });
    }, [filtersLayoutStorageKey]);
    ...
    const fetchFullPokemonData = useCallback(async (pokemonId) => {
        ...
        try {
            ...
            if (speciesData.gender_rate === 0) {
                ...
                gender = 'Female';
            } else if (speciesData.gender_rate !== -1) {
                gender = Math.random() < (speciesData.gender_rate / 8) ? 'Female' : 'Male';
            }
    ...
    const PokemonDetailCard = ({ pokemon, roundNumber }) => {
        ...
        const evolutionChain = Array.isArray(pokemon.evolutions) ? pokemon.evolutions : [];
                ...
                <div className="space-y-2.5">
                    <div className="grid grid-cols-1 sm:grid-cols-1 gap-2.5">
                       {/*  <button
                            ...
                            aria-expanded={isGuideLayoutExpanded}
                            aria-controls="random-generator-guide-panel"
                        >
                            <span className="flex items-center justify-between gap-3">
                                <span className="min-w-0">
                                    ...
                                    <span className="mt-1 block text-[11px] sm:text-xs leading-relaxed" style={{ color: colors.textMuted }}>
                                        {isGuideLayoutExpanded
                                            ? 'Hide the 3-step prompt once everyone knows the rules.'
                                            : 'Show the short mobile-friendly play flow again.'}
                                    </span>
                                </span>
                                <span
                                    className={`shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-xl transition-transform duration-300 ${isGuideLayoutExpanded ? '-rotate-90' : 'rotate-90'}`}
                    ...
                    >
                        ...
                        >
                           {/*  <div className="flex items-center gap-2 mb-2.5">
                                ...
                                <div>
                                    <p className="text-sm font-bold leading-none" style={{ color: colors.text }}>
                                        Quick party flow
                                    </p>
                                    <p className="mt-1 text-[11px] sm:text-xs" style={{ color: colors.textMuted }}>
