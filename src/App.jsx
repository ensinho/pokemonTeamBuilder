...
// Dados de tipo para análise de fraquezas/forças
const typeChart = {
...
// --- Configuração do Firebase (SUAS CHAVES ESTÃO AQUI) ---
const firebaseConfig = {
  ...
  storageBucket: "pokemonbuilder-8f80d.appspot.com", // corrigido para o domínio correto
...
export default function App() {
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            updatedPokemons = updatedPokemons.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()));
        }
    ...
    useEffect(() => {
        ...
        allTypes.forEach(attackingType => {
            ...
            teamDetails.forEach(pokemon => {
                ...
                if (pokemonMultiplier > 1) teamVulnerabilityScore++;
            });
        ...
        const finalWeaknesses = Object.entries(defensiveProfile)
            .filter(([, score]) => score >= currentTeam.length / 2 && score > 0)
    ...
    return (
        <div className="min-h-screen text-white font-sans p-4 sm:p-6 lg:p-8" style={{ backgroundColor: COLORS.background }}>
            ...
            <style>{`
                ...
                .max-h-96::-webkit-scrollbar-track { background: ${COLORS.card}; }
                .max-h-96::-webkit-scrollbar-thumb { background-color: ${COLORS.primary}; border-radius: 20px; border: 3px solid ${COLORS.card}; }
            `}</style>
        </div>
    );
}
